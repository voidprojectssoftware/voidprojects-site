...
that share one world, plus a separate at-rest cursor spring. The docs are organized by Diátaxis
mode: read for understanding, or follow to make a change.
...
API and tunable constants in `stage.ts`; per-actor config defaults in `actors/`).
